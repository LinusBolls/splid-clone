...
export class CurrenciesController {
    ...
    @Get()
    async findAll() {
        const currencies = await this.currenciesService.findAllCurrencies(new Date());
    ...
    async findOne(@Param('symbol') symbol: string) {
        const currency = await this.currenciesService.findOneCurrency(symbol, new Date());

        return this.currencyMapper.dtoFromEntity(currency);
    }

    @Patch()
    async convert(@Body() convertCurrencyDto: ConvertCurrencyRequestDto): Promise<ConvertCurrencyResponseDto> {
        let date = new Date()
        ...
        const quoteAmount = await this.currenciesService.convert(convertCurrencyDto.from, convertCurrencyDto.to, convertCurrencyDto.amount, date);

        if (quoteAmount === null) {
            throw new HttpException("Couldn't convert", HttpStatus.INTERNAL_SERVER_ERROR);
        }
